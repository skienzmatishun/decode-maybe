...
#  mean square (RMS) energy, and generates comparative visualizations between raw and 
# encrypted audio data. The script processes both encrypted and decrypted audio files, 
# saving the results in a structured format for further examination. It ensures a thorough 
# analysis of audio characteristics, facilitating the identification of patterns and 
...
def read_raw_audio(file_path):
    """Read raw audio file as a NumPy array of floats"""
    with open(file_path, "rb") as f:
        raw_data = np.frombuffer(f.read(), dtype=np.int16)  # Assuming 16-bit PCM
    return raw_data.astype(np.float32) / 32768.0  # Normalize to [-1, 1]
