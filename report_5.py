...
def parse_ml_results():
    ...
    try:
        ...
        for file in target_files:
            # Look for corresponding decryption file in all directories
            for directory in DECRYPTED_DIRS:
                potential_paths = [
                    os.path.join(directory, file),
                    os.path.join(directory, f"decrypted_{file}")
                ]
...
def generate_report(top_files):
    ...
    with open(REPORT_PATH, 'w') as f:
        ...
        f.write("and identify the most likely correctly decrypted files across multiple directories.\n\n")
        ...
        # Report on top files from each directory
        for dir_name, files in dir_groups.items():
            # Sort files by composite score
            files.sort(key=lambda x: x["composite_score"], reverse=True)
            
            f.write(f"### Top Files from {dir_name}\n\n")
            f.write("| Rank | File Name | Composite Score | Cosine Similarity | Pearson Correlation |\n")
            f.write("|------|-----------|----------------|-------------------|---------------------|\n")
            
            for i, file_data in enumerate(files[:3], 1):  # Top 3 from each directory
                metrics = file_data["metrics"]
                cosine = metrics.get("cosine_similarity", "N/A")
                pearson = metrics.get("pearson_correlation", "N/A")
        ...
        f.write("## Potential Encryption Patterns\n\n")
        f.write("Based on the top performing decrypted files, we can infer the following ")
        f.write("about the encryption algorithm:\n\n")
        
        # Analyze top files to infer encryption patterns
        if len(top_files) > 0:
            top_file = top_files[0]
            f.write(f"- The most successful decryption came from the '{os.path.basename(os.path.dirname(top_file['source_dir']))}' directory\n")
            f.write(f"- Files with high cosine similarity generally also have high Pearson correlation\n")
            if "pca_similarity" in top_file["metrics"]:
                f.write(f"- The structural patterns in the top decrypted files closely match the original\n")
            
            # Add directory-specific patterns
            f.write("\n**Directory-Specific Patterns:**\n\n")
            for dir_name in dir_groups.keys():
                f.write(f"- **{dir_name}**: ")
                if dir_name == os.path.basename(os.path.dirname(top_files[0]['source_dir'])):
                    f.write("Contains the most promising decryption approaches\n")
                else:
                    f.write("Contains alternative decryption approaches that may offer insights\n")
        
        f.write("\n## Conclusion\n\n")
        f.write("Based on our comprehensive analysis across multiple directories, the top-ranked files ")
        f.write("are the most likely to be correctly decrypted. We recommend focusing further analysis ")
        f.write("on these files, particularly the top 3, which show the strongest evidence ")
        f.write("of successful decryption.\n\n")
        
        f.write("### Next Steps\n\n")
        f.write("1. Manually inspect the audio quality of the top 10 files\n")
        f.write("2. Reverse engineer the exact encryption algorithm based on the best performing decryption methods\n")
        f.write("3. Apply the identified decryption method to the remaining encrypted files\n")
        f.write("4. Validate the decryption by comparing audio spectrograms and listening tests\n")
...
def main():
    ...
    print("="*80)
    print(f"{'Rank':<5}{'File Name':<30}{'Source Directory':<25}{'Score':<10}")
    print("-"*80)
    for i, file in enumerate(all_results[:10], 1):
        dir_name = os.path.basename(os.path.dirname(file['source_dir']))
        if not dir_name:
            dir_name = file['source_dir']
        print(f"{i:<5}{file['file_name']:<30}{dir_name:<25}{file['composite_score']:.4f}")
